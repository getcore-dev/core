...
router.get("/search-jobs", async (req, res) => {
  try {
    ...
    const result = await request.query(`
      SELECT * FROM dbo.SearchJobs(@query, @page, @pageSize)
    `);
...
router.get(
  "/updates/:updateId/pr-info",
  cacheMiddleware(1600),
  async (req, res) => {
    try {
      ...
      if (!update || !update.pull_request_url) {
        return res
          .status(404)
          .json({ error: "Update not found or no pull request URL" });
      }

      // Fetch the pull request info
      const prInfo = await utilFunctions.getPullRequestInfo(
...
router.get("/community/:communityId/jobs", async (req, res) => {
  try {
    ...
    console.error(
      "Error fetching job postings in /community/:communityId/jobs:",
      err,
    );
    res
      .status(500)
      .send("Error fetching job postings in /community/:communityId/jobs");
  }
});
...
router.get("/test", async (req, res) => {
  const simplifyJobs = await jobProcessor.processJobViteLink(
    "https://jobs.jobvite.com/splunk-careers/job/oTXtufwx?nl=1&nl=1&fr=false&utm_source=Simplify&ref=Simplify",
  );
  res.json(simplifyJobs);
});
...
router.post("/combine-duplicate-companies", async (req, res) => {
  try {
    ...
    const result = await jobQueries.combineDuplicateCompaniesAndJobs(
      companyId,
      duplicateCompanyId,
    );
    if (result.error) {
...
router.get("/create-resume/:jobId", checkAuthenticated, async (req, res) => {
  try {
    ...
    const educationExperience = await jobQueries.getUserEducationExperience(
      user.id,
    );
    ...
    const resumeData = await resumeFunctions.createResumeFromUserDataAndJob(
      fullUser,
      jobPosting,
    );
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      const coverLetterStream = await resumeFunctions.generateCoverLetter(
        fullUser,
        jobPosting,
      );
  ...
  async (req, res) => {
    try {
      ...
      if (!req.user || !req.user.id) {
        ...
        return res
          .status(401)
          .json({ message: "User not authenticated or ID not available" });
      }
      ...
      res
        .status(200)
        .json({ message: "Resume processed successfully", data: data });
    } catch (err) {
      console.error("Error processing resume:", err);
      res
        .status(500)
        .json({ message: "Error processing resume", error: err.message });
    }
  },
);

router.get(
  "/jobs/:id/similar-company",
  cacheMiddleware(2400),
  async (req, res) => {
    try {
      const id = req.params.id;
...
router.post("/job-postings", checkAuthenticated, async (req, res) => {
  try {
    ...
    if (!companyId) {
      companyObject = await jobQueries.createCompany(
        company,
        null,
        location,
        null,
  ...
  async (req, res) => {
    try {
      ...
      const jobs = await Promise.all(
        links.map((link) => jobExtractionQueue.addJob({ link })),
      );
      ...
      res
        .status(500)
        .json({ error: "An error occurred while starting job extraction" });
    }
  },
);
...
router.get("/job-extraction-progress", checkAuthenticated, async (req, res) => {
  try {
    ...
    const completedJobs = progress.filter(
    ...
    res.status(500).json({
      error: "An error occurred while fetching job extraction progress",
    });
  }
});
