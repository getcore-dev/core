...
const techJobTitles = [
  ...
  'data analyst', 'data scientist', 'data engineer', 'data analyst', 'data scientist', 'data engineer',
  'quantitative analyst', 'quantitative researcher', 'quantitative developer', 'quantitative engineer',
  'computer scientist', 'ai scientist', 'ai researcher', 'ai developer', 'ai engineer', 'ai specialist',
];
...
router.get('/tech-job-titles', cacheMiddleware(600), async (req, res) => {
  ...
  try {
    ...
    const titlesWithJobCounts = await Promise.all(potentialTitles.map(async (title) => {
      return { title, jobCount: null };
    }));

    console.log(titlesWithJobCounts);
    res.json(titlesWithJobCounts);
  } catch (err) {
