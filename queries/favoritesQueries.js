...
const favoritesQueries = {
  ...
  getFavoriteJobs: async (userId) => {
    try {
      const result = await sql.query`
        SELECT f.*, j.title, j.id, j.description, j.company_id, j.location, j.link, j.postedDate, j.salary, j.experienceLevel, c.name as company_name, c.logo as company_logo
