...
// Add this function to handle job search
function searchJobs(query) {
  fetch(`/api/search-jobs?query=${encodeURIComponent(query)}`)
    .then((response) => response.json())
    .then((data) => {
      jobSuggestions.innerHTML = "";
      if (data.length === 0) {
        jobSuggestions.innerHTML = "<p>No jobs found matching your search.</p>";
      } else {
        data.forEach((job) => {
...
function loadJobSuggestions(page = 1) {
  ...
  fetch(`/api/job-suggestions?page=${page}&limit=${jobsPerPage}`)
    ...
    .then((data) => {
      ...
      data.forEach((job) => {
        // Existing code to create and append job cards...
      });
      isLoading = false;
      currentPage++;
    })
